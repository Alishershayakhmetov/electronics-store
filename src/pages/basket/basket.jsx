...
let handleChangeSelectProduct;
let handleCheckAllChange;
let handleDeleteSelectedProductList;
...
export default function Basket() {
  ...
  handleChangeSelectProduct = (productId) => {
    setData((prevData) =>
      prevData.map((product) => {
        if (product.productId === productId) {
          axios
            ...
            .post(
              `http://localhost:8003/api/catalog/api/cart/changeSelected?productId=${productId}&value=${!product.selected}`
            );
          if (!product.selected) {
            dispatch(increment());
          } else {
  ...
  handleDeleteSelectedProductList = () => {
    for (let i = 0; i < data.length; i++) {
      if (data[i].selected) {
        dispatch(decrement());
      }
    }
    if (data.reduce((sum, item) => sum + item.selected, 0)) {
      axios
        .create({ withCredentials: true })
        .delete(
  ...
  handleDeleteProduct = (product) => {
    ...
    axios
      .create({ withCredentials: true })
      .delete(
        `http://localhost:8003/api/catalog/api/cart/deleteProduct?product=${product.productId}`,
        { data: data }
      )
      .then((response) => {
  ...
  // for all products Checkbox
  handleCheckAllChange = () => {
    const value = !data.every((product) => product.selected);
    if (!value) {
      Array(data.length)
        .fill()
        .map(() => dispatch(decrement()));
    } else {
      Array(data.length)
        .fill()
    ...
    axios
      .create({ withCredentials: true })
      .post(
        `http://localhost:8003/api/catalog/api/cart/changeSelectedList?value=${value}`,
        data
      )
      .then((response) => setData(response.data));
  };

  handleChangeAmount = (productId, amount) => {
    axios
      .create({ withCredentials: true })
      .post(
        `http://localhost:8003/api/catalog/api/cart/changeAmount?productId=${productId}&amount=${amount}`
      )
      .then((response) => setData(response.data));
  };
          ...
          <div className={style.container}>
            <div className={style.productList}>
              <div className={style.productListHeader}>
                ...
                <Button onClick={handleDeleteSelectedProductList}>
                  Delete ({data.reduce((sum, item) => sum + item.selected, 0)})
                </Button>
              </div>
              <div className={style.productListBody}>
                {data.map((product, index) => (
                  <Product key={index} product={product} lang={lang} />
                ))}
              </div>
            </div>
            <div className={style.priceBox}>
              <Block>
                ...
                <div className={style.priceShelf}>
                  <span>
                    {data.reduce((sum, item) => sum + item.selected, 0)}{" "}
                    products
                  </span>
                </div>
...
function Product({ product, lang }) {
  ...
  useEffect(() => {
    setAmount(product.amount); // Update amount state when product.amount changes
  }, [product.amount]);
  ...
  const handleChange = (e) => {
    ...
    if (parseInt(e.target.value) > 99) {
      setAmount(e.target.value.slice(0, 2));
    }
  };
  ...
  const handleBlur = (productId) => {
    ...
    handleChangeAmount(productId, result);
  };
    ...
    <div className={style.productContainer}>
      <div>
        <Checkbox
          ...
          onChange={() => handleChangeSelectProduct(product.productId)}
        />
      ...
      <div>
        ...
        <span>
          <small>
            {product.price.toLocaleString("ru-RU")}₸ x {product.amount}
          </small>
        </span>
      </div>
      <div className={style.countBox}>
        <button
          ...
          onClick={() =>
            amount === 1
              ? handleDeleteProduct(product)
              : handleChangeAmount(product.productId, product.amount - 1)
          }
        ...
        >
          <svg fill="none" stroke="" viewBox="0 0 24 24" width="18" height="18">
            <path
              d="M6 12H18"
